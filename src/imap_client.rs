...
impl ImapClient {
    ...
    pub fn new(server: &str, username: &str, password: &str) -> std::io::Result<Self> {
    ...
    pub fn send_command(stream: &mut TcpStream, command: &str, args: &str) -> std::io::Result<()> {
        stream.write_all(format!("{} {}\r\n", command, args).as_bytes())?;
        stream.flush()
    }

    /// Read the response from the server.
    pub fn read_response(stream: &mut TcpStream) -> std::io::Result<String> {
    ...
    /// Receive emails.
    pub fn receive_mail(&mut self) -> std::io::Result<Vec<String>> {
